...
def generate_text(user_input, max_new_tokens=128):
...
def process_language(lang):
    input_file = f"./Output/generate/summary/{lang}.jsonl"
    output_file = f"./Output/generate/query/{lang}.jsonl"
    results = []

    language_name = lang_names.get(lang, lang)

    with jsonlines.open(input_file) as reader:
        for item in tqdm(reader, desc=f"Generating query for {lang}"):
            if "summary" not in item or not item["summary"].strip():
                continue

            query_prompt = (
                f"The following summary is in {language_name}. Please generate a concise and specific search query "
                f"in the same language that captures the key aspects of this summary:\n\n"
                f"{item['summary']}\n\nQuery:"
            )
            generated_query = generate_text(query_prompt)
            item["generated_query"] = generated_query
            results.append(item)
